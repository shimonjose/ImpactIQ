...
def test_handoff_ack_audits(client, tmp_path, monkeypatch):
    # The ack now binds to a real baton: stub the store so it resolves.
    import impactiq.baton_store as baton_store
    from impactiq.baton_store import StoredBaton
    from impactiq.report.artifacts import ContextBaton

    stored = StoredBaton(
        baton=ContextBaton(baton_id="baton-abc123", proposed_change="x"),
        recipient="manager",
        status="sent",
        row_id="row-1",
    )
    monkeypatch.setattr(baton_store, "get_baton", lambda *a, **k: stored)
    r = client.post(
        "/handoff/ack",
...
def test_handoff_ack_rejects_unknown_baton(client, monkeypatch):
    import impactiq.baton_store as baton_store

    monkeypatch.setattr(baton_store, "get_baton", lambda *a, **k: None)
    r = client.post(
        "/handoff/ack",
        json={"baton_id": "nope", "stance": "clear", "user": "manager"},
    )
    assert r.status_code == 404
...
def test_remediate_typed_value_must_match(client):
    ...
    assert r.status_code == 403
    assert "exact proposed value" in r.json()["detail"]
    # A generic CONFIRM is not accepted either - retyping the real value is
    # the whole point of typed confirmation.
    r = client.post(
        "/action/remediate",
        json={
            "artifact": artifact,
...
def test_remediate_happy_path_executes_write_and_audits(client, tmp_path, monkeypatch):
    ...
    # The pre-write ETag read (no proposal-time etag on the legacy path).
    monkeypatch.setattr(server_mod.httpx, "get", lambda *a, **k: _GetResp())
    ...
    # Version-pinned update: the CURRENT record version, never "*" (which
    # would silently overwrite changes made after the user's preview).
    assert captured["headers"]["If-Match"] == 'W/"12345"'
...
def test_remediate_is_deterministic_even_with_mcp_configured(client, tmp_path, monkeypatch):
    """The Dataverse MCP connection never routes writes: even when it IS
    configured, the confirmed write executes as the deterministic Web API
    PATCH - no model in the execution path."""
    import dataclasses
    ...
    assert r.status_code == 200
    assert r.json()["write_channel"] == "dataverse_web_api"
    assert captured["json"] == {"statuscode": "Reopened"}
    assert _audit_events(tmp_path)[-1]["write_channel"] == "dataverse_web_api"
