...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("propose_record_fix", "record-fix proposals never offered by the front agent"),
        ...
        # (the agent must never treat the pipeline as optional and skip it).
        ...
        # them as "impacted" or imply a backfill (e.g. changing an autonumber
        # format was once presented as if already-issued numbers would change).
        ...
        ("inert Outlook draft", "draft-vs-send consequence ladder"),
...
def test_write_path_is_deterministic_no_llm_writer():
    """There must be NO model in the confirmed write path: an LLM writer
    (the Dataverse-MCP agent route) is a prompt-injection surface and cannot
    prove exactly-one-call. Confirmed writes are a single deterministic Web
    API call - version-pinned (real ETag, never ``If-Match: *``) - executed
    against the SERVER-stored proposal."""
    import inspect

    import impactiq.server as server

    assert not hasattr(server, "_write_via_dataverse_mcp")  # writer agent gone
    patch_src = inspect.getsource(server._write_via_patch)
    assert '"If-Match": "*"' not in patch_src  # unconditional overwrite banned
    assert "412" in patch_src                   # concurrent change detected
    rem_src = inspect.getsource(server.remediate)
    assert "proposals.consume" in rem_src       # server-stored proposal binding


def test_pending_caches_evict_oldest_not_newest():
    """Pending-state eviction used dict.popitem() (LIFO), dropping the
    most-recently-suspended entry while the comment promised 'oldest-first' -
    a recent run could be evicted and its approval lost. The pending-run cache
    and the proposal store both evict FIFO (oldest) via next(iter(...))."""
    import inspect

    import impactiq.proposals as proposals
    import impactiq.server as server

    for fn in (server._stash_pending_run, proposals.register):
