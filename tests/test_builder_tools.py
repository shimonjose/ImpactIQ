...
def test_fix_is_a_proposal_not_an_apply(monkeypatch):
    ...
    # The stashed proposal lives in the one-time store (consume also cleans up).
    from impactiq import proposals

    stored = proposals.consume(fix_id, "sandbox_fix")
    assert stored["artifact"]["ops"] == ops
...
def test_apply_endpoint_requires_tap_and_known_id(monkeypatch):
    import impactiq.builder.gate as gate
    ...
    # Role gate passes -> an unknown/expired proposal id is a 404.
    monkeypatch.setattr(
        gate, "assert_builder_permission", lambda s, ua=None, fresh=False: None
    )
    r = client.post("/action/sandbox_fix", json={"fix_id": "fix-x", "confirmed": True})
    assert r.status_code == 404


def test_apply_endpoint_authorizes_before_consuming(monkeypatch):
    """A refused Apply must NOT consume (destroy) the pending proposal:
    authorization runs first, so the rightful owner can still apply it
    afterwards."""
    import impactiq.builder.gate as gate
    import impactiq.server as server
    from fastapi.testclient import TestClient
    ...
    client = TestClient(app)
    fix_id = server._stash_pending_fix(
        [{"op": "set_flow_state", "flow": "x", "state": "off"}], "t", "r"
    )

    def _deny(settings, ua=None, fresh=False):
        raise BuilderRefusal("you don't hold the 'ImpactIQ Builder' security role")

    monkeypatch.setattr(gate, "assert_builder_permission", _deny)
    r = client.post("/action/sandbox_fix", json={"fix_id": fix_id, "confirmed": True})
    assert r.status_code == 403
    # Still there - the denied caller could not destroy it.
    stored = proposals.consume(fix_id, "sandbox_fix")
    assert stored["artifact"]["title"] == "t"
...
def test_resubmit_tool_proposes_never_executes(monkeypatch):
    ...
    stored = proposals.consume(out["resubmit_id"], "resubmit_run")
    assert stored["artifact"]["flow_resource"] == "res-1"
