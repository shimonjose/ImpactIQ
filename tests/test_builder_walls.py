...
These tests pin the sandbox carve-out's refusal behaviour, checked before any
fix logic runs: no sandbox URL, same-environment, and missing solution scope
must all refuse loudly. The walls are the contract that lets the project
tolerate configuration writes at all.
"""
...
def test_refuses_same_environment_even_with_cosmetic_differences():
    ...
    s = _settings(build_dataverse_url="HTTPS://EXAMPLE.CRM.DYNAMICS.COM/")
