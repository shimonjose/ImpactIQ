...
interface ConversationState {
  ...
  pendingAssist: {
    ...
    response_id: string;
    resume_path: string; // "/agent/approve" (unified) or "/agent/approve"
...
// The unified capability-aware agent (estate engine + records + all Work IQ +
// knowledge base + the deep multi-agent pipeline as a tool, in one loop) is
// the default surface for every message — the agent itself decides when to
// convene the specialist pipeline; there is no pre-router.
...
// durable bot-state storage keyed by user+conversation; same shape.)
...
// Maps a user identity (aadObjectId / display name, lowercased) -> the
// proactive conversation id stored for them, so we can push the handoff card
// to a manager who has interacted with ImpactIQ at least once.
...
async function rememberConversation(context: TurnContext): Promise<string | undefined> {
  try {
    ...
    for (const k of [from?.aadObjectId, from?.name]) {
...
// Best-effort resolve a handoff recipient (UPN or display name) to a stored
// conversation id. Returns undefined if the manager hasn't messaged the bot.
function resolveConvId(recipient: string): string | undefined {
  const r = (recipient || "").trim().toLowerCase();
  if (!r) return undefined;
  return convIdByKey.get(r) ?? convIdByKey.get(r.split("@")[0]);
}

// Headers for every bridge call: JSON + the shared secret (when configured).
// Empty locally (guard is a no-op); set in production so only the surface can
// reach the bridge API.
function bridgeHeaders(userToken?: string): Record<string, string> {
  const h: Record<string, string> = { "Content-Type": "application/json" };
  if (config.bridgeKey) h["X-ImpactIQ-Key"] = config.bridgeKey;
...
async function bridgePost(path: string, body: unknown, userToken?: string): Promise<any> {
  const resp = await fetch(`${config.bridgeUrl}${path}`, {
    method: "POST",
    headers: bridgeHeaders(userToken),
...
// abort it. `bridgeTimeoutMs` is now the TOTAL wait budget we enforce, not a
...
function approvalCard(
  ...
  return {
    ...
    body,
    actions: [
      // Plain Action.Submit only: the msteams messageBack wrapper looked
      // nice in Teams (echoes the tap as the user's message) but Copilot
      // drops the value payload for it — the tap then reaches the MODEL as
      // text instead of the action handler. Reliability wins.
...
agentApp.onConversationUpdate("membersAdded", async (context: TurnContext) => {
  ...
  // message — a static greeting wasn't useful; the first real reply (with its
...
async function handleCardAction(
  ...
  switch (value.action) {
    ...
    case "deny_assist": {
      ...
      const resume = async () => {
        const conv = (context.activity.conversation?.id || "").slice(-8);
        try {
...
agentApp.onActivity(ActivityTypes.Message, async (context: TurnContext) => {
  ...
  // Teams/Copilot conversation ids can carry a per-message ";messageid=..."
  // suffix — strip it, or state and dedupe silently fragment per message.
  const conversationId = (context.activity.conversation?.id ?? "").split(";")[0];
  console.log(
...
async function processQuestion(
  ...
  // out and redelivers (the root cause of duplicate answers in Copilot).
  const send = convId
    ? proactiveSender(convId)
    : async (a: any) => {
        await context.sendActivity(a);
  ...
  const run = async () => {
    const conv = conversationId.slice(-8);
    try {
...
async function renderAssistResult(
  ...
  if ((!res.text || !String(res.text).trim()) && !showedSomething) {
    // Should be rare now (the loop nudges the model to answer) — be honest
    // rather than a bare "Done." when it still happens.
    await send(
