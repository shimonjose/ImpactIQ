...
* ``set_status`` is best-effort: the manager holds only User-level Write, so
  cannot patch a row B owns. The half-loop therefore records the manager's
  ack/resume as *audit events*, not row writes; status mutation is reserved
  for the owner (B) / a future Org-scoped grant.
...
def set_status(
    ...
    whether the patch landed. Audit-logged either way. The half-loop does NOT
    rely on this — it records the manager's ack/resume as audit events."""
    try:
