...
def _iter_all_actions(actions: dict | None):
    ...
    case - not at the top level. Walking only ``definition.actions`` would
    miss those, so a flow that creates a record only inside a nested
    ``If <condition>`` branch would look like it writes nothing.
    """
