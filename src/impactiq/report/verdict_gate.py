"""Deterministic post-adjudication verdict gate (additive).
...
* ``shadow`` (default — also what unset means): compute every would-be action,
  ...
  run without the gate. This is the safe dev default while the eval corpus
  proves the gate strips only genuinely-ungrounded content.
...
1. **Citation grounding.** A report ``citations[]`` entry survives only if its
   ...
   citations are — which is exactly why this starts in shadow: if a legitimate
   KB hit fails to surface as an annotation we will SEE the over-strip in the
...
name on no evidence. This conservatism is *why* enforce mode is gated behind the
eval corpus rather than flipped on blind.
"""
...
def _evaluate(
    ...
    for r in results or []:
        ...
        # enforce mode it would have dropped legitimate SOP/policy
