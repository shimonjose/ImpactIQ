...
# The only properties a fix may change - anything else is refused.
...
def solution_id(client: SandboxClient, unique_name: str) -> str:
    safe_name = unique_name.replace("'", "''")
    data = client.get(
        "solutions",
        {"$select": "solutionid,uniquename", "$filter": f"uniquename eq '{safe_name}'"},
    )
    rows = data.get("value", [])
    if not rows:
...
def failed_run_details(
    ...
    the DELEGATED user token (run payloads are content, not structure, and
    content reads always run as the user).
...
def table_schema(client: SandboxClient, table: str) -> dict:
    ...
    try:
        ent = client.get(
            f"EntityDefinitions(LogicalName='{safe_table}')",
        ...
        rows = client.get(
            ...
            {
                "$select": "LogicalName,EntitySetName",
                "$filter": f"EntitySetName eq '{safe_table}'",
            },
        ).get("value", [])
        if not rows:
            raise BuilderRefusal(
