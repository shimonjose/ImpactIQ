...
Building the estate (Dataverse metadata + flows + roles reads → normalized
fragment) from scratch takes tens of seconds before the model sees a single
token. The snapshot is cached here per (org, solution) and reused within the
TTL, so follow-up questions start reasoning immediately.
...
def get_estate_cached(
    ...
    # use_cache=False is load-bearing: THIS module owns estate freshness (the
    # TTL and invalidate()). build_estate's own process-global
