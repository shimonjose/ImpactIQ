...
class Settings:
    ...
    # Expected audience for the forwarded Teams user token. The surface obtains
    # that token for the OBO middle-tier app (impactiq-workiq), so that app's id
    # is the default expected audience; set IMPACTIQ_TOKEN_AUDIENCE to override
    # when the tenant issues a custom App ID URI. When neither is available the
    # signature, issuer, and expiry are still enforced; only audience binding is
    # skipped.
    impactiq_token_audience: str | None = field(
        default_factory=lambda: _get("IMPACTIQ_TOKEN_AUDIENCE")
    )

    def expected_token_audiences(self) -> list[str]:
        """Accepted ``aud`` values for a forwarded user token (both the bare
        app-id and the ``api://`` App ID URI form). Empty when no audience is
        configured, which disables only the audience check."""
        aud = self.impactiq_token_audience or self.impactiq_workiq_client_id
        if not aud:
            return []
        forms = {aud}
        forms.add(aud[len("api://"):] if aud.startswith("api://") else f"api://{aud}")
        return list(forms)
