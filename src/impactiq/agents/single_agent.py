...
# NOTE: This is the legacy single-agent baseline (CLI-only, reached via
# cli.py's `ask`). Its artifact contract is the original SIX-type world
# (dev_ticket / reuse_blueprint / draft_teams_intro + the three real actions).
# The CURRENT production path is multi_agent.ask_multi, whose adjudicator
# narrowed `generated_artifact` to the THREE real, executable actions
# (manager_handoff / remediation_proposal / backfill_blueprint) and treats a
# dev_ticket as draftable TEXT, not an action. The divergence below is that
# legacy-vs-current split, not a contradiction in the live agent.
SYSTEM_INSTRUCTIONS = """\
...
3. **Primary move (mandatory on every anchor)**: call `walk_anchor` to get
   the §4.0 dependency neighbourhood. This is the suspect population (DIAGNOSE)
...
* **Defect-vs-expected reconciliation (§5.7)**: if the structural walk
...
* When the radius is sparse (causal count = 0), say so honestly. Note that
  per-record data backfill is out of phase-3 scope.
...
# Disclosure gate for Work IQ signals (safety-critical, §6.3)
...
def _build_workiq_mail_tool(settings: Settings) -> MCPTool | None:
    ...
    headless (an MCP approval pause here would hang, the same class of failure
    as the validated-write EOF). Runs OBO as the signed-in user, so the draft
    lands in *their* Outlook Drafts."""
