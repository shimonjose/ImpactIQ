...
# ~200s - past the default cap, which would DROP the context finding (including
# evidence it had just retrieved) milliseconds before it completed.
...
def _salvage_plan(raw: Any) -> OrchestratorPlan:
    ...
    A blanket fallback (``except: OrchestratorPlan()``) would discard the
            ...
            if isinstance(report_dict, dict):
                ...
                if critic_enabled():
                    ...
                    if trigger:
                        ...
                        # MUTATIONS FAIL CLOSED: a write artifact ships only
                        # with the verifier's explicit approval. A refusal OR
                        # a verifier error/empty critique withholds the write
                        # (the narrative still ships with a caveat) - an
                        # unverified mutation proposal must never reach the
                        # confirm card. Read-only conclusions stay fail-open.
                        wtype = carries_write_artifact(report_dict)
                        if wtype and critique.get("write_artifact_safe") is not True:
                            reason = str(critique.get("write_concern") or "").strip() or (
                                "the verifier could not confirm the proposed "
                                "change (verification unavailable) - re-ask to "
                                "regenerate the proposal"
                            )
                            report_dict = apply_write_deny(report_dict, reason)
                            tool_names.append("critic:write_denied")
                            print(f"[critic] write artifact '{wtype}' WITHHELD: "
                                  f"{reason}", flush=True)
