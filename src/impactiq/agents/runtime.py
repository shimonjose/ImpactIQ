...
The agent loop uses **only** ``AIProjectClient`` - the prompt-agent pattern
(``project.agents.create_version`` + ``openai.responses.create``) lives
entirely in ``azure-ai-projects``, so no separate ``AgentsClient`` from
``azure-ai-agents`` is needed. ``ensure_os_trust`` keeps the Windows OS
