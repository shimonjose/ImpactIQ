...
Best-effort: a skeptic failure FAILS OPEN (report unchanged) — a write artifact
is still only a PROPOSAL the user must tap/type to confirm, so the human confirm
remains the real gate. The separate EXECUTION-time check (``update_record`` truly
in tool_names, not a trusted run_status) lives in ``server._verify_record_write``.
"""
...
def run_skeptic(
    ...
    """Single skeptic LLM hop (no tools). Returns the parsed critique, or a
    fail-OPEN default ({} → verdict holds, write safe) on any error."""
    try:
        turn = run_agent_turn(
            project_client,
